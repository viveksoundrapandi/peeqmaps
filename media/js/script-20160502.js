			...
			if (data.length > 0) {
				for (var i = 0; i < data.length; i++) {
					...
					if (circleBounds.contains(latlng) == true && google.maps.geometry.spherical.computeDistanceBetween(current_circle, latlng) <= circle.getRadius()) {
						createMarker(lat, lng, company_size, funding, industry, tags, distanceFromHome, company_name, website, careersPage, careersLogo, websiteLogo, linkedinLogo, facebookLogo, twitterLogo, angellistLogo, githubLogo, crunchbaseLogo, blogLogo, stackoverflowLogo, ventureloopLogo, address);

						exportTable += '<tr><td><input type="checkbox" class="" name="company" checked/></td> <td>' + company_name + '</td><td>' + distanceFromHome + '</td><td>' + address + '</td><td>' + company_size + '</td>';
						/*exportTable+='<tr><td><input type="checkbox" class="" name="company" checked/></td><td>'+
										'sdasd' ;*/
...
function createCircle() {
	...
	var smallCircleRadius = (circle.getRadius()) * 5 / 100;
...
//###### Marker Creation ####
function createMarker(lat, lng, company_size, funding, industry, tags, distanceFromHome, company_name, website, careersPage, careersLogo, websiteLogo, linkedinLogo, facebookLogo, twitterLogo, angellistLogo, githubLogo, crunchbaseLogo, blogLogo, stackoverflowLogo, ventureloopLogo, address) {
	if (distanceFromHome) {
		var dist = parseFloat(distanceFromHome / 1000);
		distanceFromHome = dist.toFixed(1) + ' Km';
	}
	$("#rightContainer .panel-default").append('<div class="panel-heading"><a data-position=' + markers.length + '>' + company_name + ':</a></div>');
		...
		geocoder.geocode({
			...
			if (status == google.maps.GeocoderStatus.OK) {
				if (results[0]) {
					...
					if (parseFloat(distanceFromHome.split(" km")[0]) < 2) {
					...
					if (parseFloat(distanceFromHome.split(" km")[0]) < 150) {
						directionsService.route(request, function(result, status) {
							...
							infoWindow.setContent('<b>' + company_name + '</b><br>' + address + '<br>' + company_size + ' ' + industry + ' ' + funding + '<br>' + tagsButton + '<br><br>' +
								websiteLogo + ' ' + linkedinLogo + ' ' + facebookLogo + ' ' + twitterLogo + ' ' + angellistLogo + ' ' + githubLogo + ' ' + crunchbaseLogo + ' ' + blogLogo + ' ' +
								ventureloopLogo + '<br><br>' + careersLogo + ' ' + stackoverflowLogo + '<br><br>' + distanceFromHome + ' ' + traelTime);
...
$(document).ready(function() {
	...
	$("#rightContainer .panel-default").on("click", "a", function() {
		open_marker_info(+$(this).data('position'));
	});
		...
		$.ajax({
			...
			success: function(data) {
				...
				for (var i = 0; i < 1; i++) {
					...
					map.setCenter(selectedLocation);
					createCircle();
					createMarker(lat, lng, company_size, funding, industry, tags, distanceFromHome, company_name, website, careersPage, careersLogo, websiteLogo, linkedinLogo, facebookLogo, twitterLogo, angellistLogo, githubLogo, crunchbaseLogo, blogLogo, stackoverflowLogo, ventureloopLogo);
